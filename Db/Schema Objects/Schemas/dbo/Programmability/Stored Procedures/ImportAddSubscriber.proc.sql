...
AS 
  BEGIN 
      ...
      WHEN MATCHED AND [Target].IdSubscribersStatus IN (1, 2, 7) THEN 
        UPDATE SET [Target].Gender = (CASE WHEN @Gender <> '*' THEN @Gender ELSE [Target].Gender END), 
                   [Target].UTCBirthday = (CASE WHEN @UTCBirthday <> CONVERT(VARCHAR(8), CONVERT(varchar,YEAR(GETUTCDATE())+1), 112)+'0101' THEN @UTCBirthday ELSE [Target].UTCBirthday END), 
                   [Target].IdCountry = (CASE WHEN @IdCountry <> -1 THEN @IdCountry ELSE [Target].IdCountry END), 
                   [Target].FirstName = (CASE WHEN @FirstName <> '**MISSINGCOLUMN**' THEN @FirstName ELSE [Target].FirstName END), 
                   [Target].LastName = (CASE WHEN @LastName <> '**MISSINGCOLUMN**' THEN @LastName ELSE [Target].LastName END),
      ...
      WHEN NOT MATCHED AND @MaxSubscribers > @AmountSubscribers THEN 
        ...
        VALUES (@IdUser, 
                ...
                (CASE WHEN @UTCBirthday <> CONVERT(VARCHAR(8), CONVERT(varchar,YEAR(GETUTCDATE())+1), 112)+'0101' THEN @UTCBirthday ELSE NULL END), 
                (CASE WHEN @IdCountry <> -1 THEN @IdCountry ELSE NULL END), 
                (CASE WHEN @FirstName <> '**MISSINGCOLUMN**' THEN @FirstName ELSE NULL END), 
                (CASE WHEN @LastName <> '**MISSINGCOLUMN**' THEN @LastName ELSE NULL END), 
                1, 
                2, 
                GETUTCDATE()) 
      OUTPUT $action, 
